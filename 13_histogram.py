...
mask[1500:2000, 1000:2000] = 255 # beyaz
plt.figure()
plt.imshow(mask, cmap="gray")
...
plt.figure()
plt.imshow(mask_img_vis, cmap="gray")


masked_img = cv2.bitwise_and(golden_gate, golden_gate, mask=mask) 
mask_img_hist = cv2.calcHist([golden_gate], channels=[0], mask=mask, histSize=[256], ranges=[0,256])
plt.figure()
plt.plot(mask_img_hist)
